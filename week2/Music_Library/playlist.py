...
class Playlist:
    ...
    def __str__(self): 
        result = ""
        for each_song in self.collection:
            result = result + each_song.artist +" "+ each_song.title +" - "+ self.convert_seconds_to_str_minutes(each_song.length) + "\n"
        return result
    ...
    def load(file_name):
        ...
        for each_song in returndata['songs']:
            song = Song(each_song['title'],each_song['artist'],each_song['album'], each_song['rating'], each_song['length'], each_song['bitrate'])
            playlist.add_song(song)
        return playlist
def main():
    playlist = Playlist("MyPlaylist")
    song = Song("Sweet Child O'mine", "Guns N' Roses", "Appetite for Destruction", 5, 360, 320 )
    song1 = Song("Sweet", "Guns N' Roses", "Appetite for Destruction1", 2, 306, 120 )
    playlist.add_song(song)
